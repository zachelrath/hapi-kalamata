...
var kalamata = module.exports = function(_server_, _options_) {
	server = _server_;
	options = _options_;
...
kalamata.expose = function(model, _opts_) {
	...
	beforeHooks = hooks[modelNameLower].before;
	afterHooks = hooks[modelNameLower].after;
	...
	function configureEndpoints() {
		...
		server.route({
			method: 'GET',
			path: options.apiRoot + opts.endpointName, 
		...
		// GET /:endpointName/{identifier}
		server.route({
			method: 'GET',
			path: options.apiRoot + opts.endpointName + '/{identifier}', 
			handler: function(req, res) {
		...
		server.route({
			method: 'GET',
			path: options.apiRoot + opts.endpointName + '/{identifier}/{relation}', 
			handler: function(req, res) {
				var mod = new model(getModelAttrs(req));
				mod.fetch({
		...
		// POST /:endpointName/{identifier}/{relation}
		server.route({
			method: 'POST',
			path: options.apiRoot + opts.endpointName + '/{identifier}/{relation}', 
			handler: function(req, res) {
				...
				promise.then(function(m) {
					if(req.payload[rId]) {
						...
						return (new rModel(req.payload)).fetch().then(function(rMod) {
							if(rMod) {
								relMod = rMod;
								...
								throw new Error('Create relationship failed: ' +
													'Could not find ' + rel +
													' model ' + JSON.stringify(req.payload));
							}
						});
					} else {
						throw new Error('Create relationship failed: ' +
		...
		// PUT /endpointName/{identifier}
		server.route({
			method: 'PUT',
			path: options.apiRoot + opts.endpointName + '/{identifier}', 
			handler: function(req, res) {
				new model(getModelAttrs(req)).fetch().then(function(m) {
					if(m) m.set(req.payload);
		...
		// DELETE /:endpointName/{identifier}
		server.route({
			method: 'DELETE',
			path: options.apiRoot + opts.endpointName + '/{identifier}', 
			handler: function(req, res) {
		...
		// DELETE /:endpointName/{identifier}/{relation}
		server.route({
			method: 'DELETE',
			path: options.apiRoot + opts.endpointName + '/{identifier}/{relation}', 
			handler: function(req, res) {
		...
		// DELETE /:endpointName/{identifier}/{relation}/:rIdentifier
		server.route({
			method: 'DELETE',
			path: options.apiRoot + opts.endpointName + '/{identifier}/{relation}/:rIdentifier', 
			handler: function(req, res) {
				var rel = req.params.relation;
				var rModel = modelMap[rel];
				var rId = identifierMap[rel];
	...
	function getRunRelatedHookFunc(relHooks,req,res) {
		return function(qb) {
	...
	function getWithRelatedArray(related, req, res) {
		...
		for(var i in related) {
			...
			if(relHooks) {
				var relObj = {};
				relObj[r] = getRunRelatedHookFunc(relHooks,req,res);
	...
	function sendResponse(response, sendData) {
		if(!response.headersSent) response(sendData);
	}
